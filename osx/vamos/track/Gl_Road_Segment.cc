...
Segment_Iterator::operator ++ ()
{
  ...
  if (((m_position == END) && m_segment.is_last_segment ())
	  ...
	  && (m_strip != Gl_Road_Segment::RIGHT_KERB))
	{
	  ...
	  m_coordinates = 
        Three_Vector (-across * sin (m_angle), across * cos (m_angle), up);
	}
  else
    {
...
Segment_Iterator::increment_distance ()
{
  ...
  if ((m_segment.length () - m_distance - m_resolution) 
      < (0.01 * m_resolution))
    {
      m_distance = m_segment.length ();
      m_position = END;
    }
  else if (m_strip == Gl_Road_Segment::LEFT_KERB)
    increment_kerb_distance (*(m_segment.mp_left_kerb));
  else if (m_strip == Gl_Road_Segment::RIGHT_KERB)
    increment_kerb_distance (*(m_segment.mp_right_kerb));
  else 
  ...
  if ((m_segment.pit ().active ())
      && (m_distance > m_segment.pit ().split_or_join ())
      && (m_distance - m_segment.pit ().split_or_join () < m_resolution))
    m_distance = m_segment.pit ().split_or_join ();
}
...
(double resolution, 
 ...
 const std::vector <Braking_Marker*>& braking_markers)
  : Road_Segment (length, radius, 10.0, 10.0, 20.0, 20.0),
    m_gl_texture_name (0),
    m_gl_list_id (0),
    m_texture_offsets (materials.size ()),
    mp_iterator (new Segment_Iterator (*this, resolution))
{
  ...
  set_banking (bank_points, end_bank, bank_pivot);
