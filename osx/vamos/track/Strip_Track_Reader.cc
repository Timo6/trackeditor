...
Strip_Track_Reader::on_start_tag (const Vamos_Media::XML_Tag& tag)
{
  ...
  else if (path () == "/track/racing-line/show")
    {
      mp_road->mp_track->m_draw_racing_line = true;
    }
  ...
  if (path () == "/track/road/left-kerb/start/transition")
	{
	  m_bools [1] = true;
	}
  else if (path () == "/track/road/left-kerb/end/transition")
	{
	  m_bools [2] = true;
	}
  else if (path () == "/track/road/right-kerb/start/transition")
	{
	  m_bools [3] = true;
	}
  else if (path () == "/track/road/right-kerb/end/transition")
	{
	  m_bools [4] = true;
	}
  else if (label () == "sky")
  ...
  if (label () == "road")
	{
	  ...
	  std::vector <Two_Vector> vec;

	  if (!m_bools [0])
		{
		  m_doubles [2] = 0.0;
		}
	  Gl_Road_Segment* segment = new Gl_Road_Segment (m_doubles [0], 
												m_doubles [1], 
												m_doubles [2],
                                                m_doubles [20],
												m_point_vectors [0], 
	...
	{
      Direction side = m_bools [5] ? RIGHT : LEFT;
      m_braking_markers.
...
Strip_Track_Reader::on_data (std::string data)
{
  ...
  else if (path () == "/track/start-slope")
    {
      double slope;
      is >> slope;
      mp_road->set_start_slope (slope);
    }
  else if (path () == "/track/racing-line/distance-factor")
    is >> mp_road->mp_track->m_racing_line.m_distance_factor;
