...
//=============================================================================
Banking::Banking ()
  : m_start_angle (0.0), m_end_angle (0.0), m_pivot_from_center (0.0),
    mp_bank_curve (new Vamos_Geometry::Spline)
{
}

Banking::~Banking ()
{
  delete mp_bank_curve;
}

void 
Banking::set (const TPoints& angles, double end_angle, double pivot_from_center)
{
  m_angles = angles;
...
{ 
  return Vamos_Geometry::deg_to_rad (mp_bank_curve->interpolate (along));
}
...
Banking::set_start (double start_angle, double length)
{
  ...
  m_angles.clear ();
  m_angles.insert (m_angles.begin (), Two_Vector (0.0, m_start_angle));
  m_angles.push_back (Two_Vector (length, m_end_angle));
  
  delete mp_bank_curve;
  mp_bank_curve = new Vamos_Geometry::Spline (m_angles, 0.0, 0.0);
}
  ...
  if (m_last_segment)
	{
	  ...
	  // Force the elevation curve to return to zero.
	  mp_elevation_curve->load (Two_Vector (start_distance + m_length, 0.0)); 
	}
}
...
Three_Vector 
Road_Segment::normal (double along, double from_center, const Three_Vector& bump) const
{
  assert (mp_elevation_curve != 0);
...
Three_Vector 
Road_Segment::normal (double along, double from_center) const
{
  return normal (along, from_center, Three_Vector::Z);
}
                         ...
                         const std::vector <double>& texture_offsets)
{
  ...
  m_start_angle = branch (start_angle, 0.0);
...
Road_Segment::left_racing_line_width (double distance) const
{
  ...
  if ((m_racing_line_adjustment_distance < 0.0)
      || (distance -start_distance () < m_racing_line_adjustment_distance))
