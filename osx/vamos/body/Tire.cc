								...
								double patch_speed, double current_camber)
{
  ...
  double slip_x = sigma / m_peak_slip;
  m_peak_slip_angle = peak_slip (By, Cy, Ey, Shy, m_peak_slip_angle);
  double slip_y = alpha / m_peak_slip_angle;
  m_peak_aligning_angle = peak_slip (Bz, Cz, Ez, Shz, m_peak_aligning_angle);
  double slip_z = alpha / m_peak_aligning_angle;

  double slip_xy = Three_Vector (slip_x, slip_y, 0.0).magnitude ();
  double Fx = pacejka_equation (sign (slip_x) * slip_xy * m_peak_slip, 
                                Bx, Cx, Dx, Ex, Shx, 0.0);
  double Fy = pacejka_equation (sign (slip_y) * slip_xy * m_peak_slip_angle, 
                                By, Cy, Dy, Ey, Shy, Svy);

  double slip_xz = Three_Vector (slip_x, 0.0, slip_z).magnitude ();
  double Mz = pacejka_equation (slip_xz * m_peak_aligning_angle, 
                                Bz, Cz, Dz, Ez, Shz, Svz);

  if (slip_xy > 1.0e-6)
	{
  	  Fx *= std::abs (slip_x) / slip_xy;
      Fy *= std::abs (slip_y) / slip_xy;
    }
  if (slip_xz > 1.0e-6)
      Mz *= slip_z / slip_xz;
