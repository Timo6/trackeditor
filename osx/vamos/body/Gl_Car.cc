...
{ 
  ...
  b_stream.str ("");
  b_stream << "Air Density " << std::setprecision (3)
		   << m_air_density << " kg/m^3";
  draw_string (b_stream.str (), 2.8, 0.2);


  b_stream.str ("");
  b_stream << "Slip Ratios (%)";
  draw_string (b_stream.str (), 2.8, 1.8);

  b_stream.str ("");
  b_stream.setf (std::ios::fixed);
  b_stream << std::setprecision (3) << m_wheels [0]->slip ().x;
  draw_string (b_stream.str (), 2.8, 1.4);

  b_stream.str ("");
  b_stream << std::setprecision (3) << m_wheels [1]->slip ().x;
  draw_string (b_stream.str (), 3.6, 1.4);

  b_stream.str ("");
  b_stream << std::setprecision (3) << m_wheels [2]->slip ().x;
  draw_string (b_stream.str (), 2.8, 1.0);

  b_stream.str ("");
  b_stream << std::setprecision (3) << m_wheels [3]->slip ().x;
