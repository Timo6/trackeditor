...
Car::set_robot_parameters (double slip_ratio,
                           const std::vector <double>& deceleration,
...
Wheel* 
Car::wheel (int wheel_index) const
{
...
// Return a number (0.0 to 1.0) that describes the front/rear weight
// balance.  1 means the front and rear weights are equal.  0 means
// all the weight is on the front or rear tires -- we don't indicate
// which.
double
Car::balance () const
{
  double front_force = 0.0;
  double rear_force = 0.0;
  for (std::vector <Wheel*>::const_iterator it = m_wheels.begin ();
       it != m_wheels.end ();
       it++)
    {
      if ((*it)->position ().x > m_chassis.center_of_mass ().x)
        front_force += (*it)->normal_force ();
      else
        rear_force += (*it)->normal_force ();
    }

  const double total_force = front_force + rear_force;
  return std::min (front_force, rear_force) / (0.5 * total_force);
}
