...
namespace Vamos_Body
{
  ...
  private:
    ...
    {
      double slip_ratio;
      std::vector <double> deceleration;
  ...
  public:
    ...
    { return std::abs (m_steer_key_control.value ()) / m_max_steer_angle; }
    ...
    // Return a number (0.0 to 1.0) that describes the front/rear
    // weight balance.  1 means the front and rear weights are equal.
    // 0 means all the weight is on the front or rear tires -- we
    // don't indicate which.
