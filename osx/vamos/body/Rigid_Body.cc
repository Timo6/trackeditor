...
Rigid_Body::propagate (double time)
{
  ...
  if (m_cm_force.magnitude () > 1.0e-9)
    std::cerr << m_cm_force << std::endl;
...
// Return the sum of the values that get multiplied by v^2 to get the
// drag force for each drag-producing particle.
double
